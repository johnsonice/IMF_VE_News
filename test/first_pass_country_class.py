...
if __name__ == "__main__":
    ...
    with open(files_read_csv, 'w+') as out_file:
        for i in random_sample_indices:
            ...
            # Lemmatize - test to see if necessary - could take a lot longer?
            tokenized_pos = nltk.pos_tag(tokens_stemmed)
            tokenized_lemmatized = [lemmatizer.lemmatize(el[0], get_wordnet_pos(el[1])) for el in tokenized_pos]
            ...
            for i in range(len(tokenized_lemmatized)):
                inc_search = False
                inc_other = False
                tok = tokenized_lemmatized[i]
            ...
            if thick:
                glued = " ".join(tokenized_lemmatized)
