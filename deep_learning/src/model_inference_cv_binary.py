...
if __name__ == '__main__':
    crisis_version = 'kr_news_filter_w12' #'kr_news_filter_w24' #'kr_w12' #or 'rr' or 've_q' or 'kr'
    crisis_version_w24 = 'kr'
    n_class = '_c2' ## or ''   or '_c2'
...
#                                      'train_data_{}.pkl'.format(crisis_version))
    ...
    cv_data_path = os.path.join(config.CRISIS_DATES,
                                      'train_data_cv_{}{}.pkl'.format(crisis_version,n_class))
    ## load cv data
    ...
    trained_model_paths={i:os.path.join(config.TRAINED_DEEP_MODEL,
                                    'cv_nn_model',
                                    'weights_cv_{}{}_{}'.format(crisis_version,n_class,i)) for i in cv_data.keys()}
    
    label_map_path = os.path.join(config.TRAINED_DEEP_MODEL,
                                  'cv_nn_model',
                                  'label_map_{}{}.json'.format(crisis_version,n_class))
    
    model_eval_result = os.path.join(config.TRAINED_DEEP_MODEL,
                                  'cv_nn_model',
                                  'eval_{}{}.txt'.format(crisis_version,n_class))
   ...
   #%% 
    ...
    for i in cv_data.keys():
        ...
        msg = '{}\nprecision:{:.3f} ; recall:{:.3f} ; f2:{:.3f}'.format(cv_data[i]['countries'],precision,recall,f2)
...
#        print('precision:{} ; recall:{} ; f2:{}'.format(precision,recall,f2))
    ...
    msg = 'Overall results\nprecision:{:.3f} ; recall:{:.3f} ; f2:{:.3f}'.format(precision,recall,f2)
    write_to_txt(model_eval_result,msg,over_write=False)
