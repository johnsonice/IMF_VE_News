...
def visualize(model, output_path):
    ...
    tf_config = projector.ProjectorConfig()
    embed = tf_config.embeddings.add()
    embed.tensor_name = 'w2x_metadata'
    embed.metadata_path = meta_file

    # Specify the width and height of a single thumbnail.
    projector.visualize_embeddings(writer, tf_config)
...
#        print("Please provice model path and output path")
    ...
    model_path = config.W2V
    output_path = os.path.join(config.VS_MODELS,"viz")
