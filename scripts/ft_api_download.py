...
logname = os.path.join("FT_Log_{}.log".format(str(dt.datetime.today().strftime("%m.%d.%Y"))))
...
def get_params(filename, history,since=None):
    ...
    api_key = key.strip()
    if history:
        raise('read historical link not implemented ! ')
    else:
    ...
    query_params = {
        'proxies':{'http': "socks5://intsquid:8000",
                   'https': "socks5://intsquid:8000",
                   'https': "wsproxy:8080"},  ## need this for local runs 
        'base_url':main_url,
        'api_key': api_key,
...
def download_docs(args):
    ...
    last_link = pages[-1]
    with open(os.path.join(args.data_dir,"LAST_LINK.txt"), 'w') as f:
...
def ft_api_args():
    ...
    parser.add_argument('-dd', '--data_dir', action='store', dest='data_dir',                
                        default=os.path.join("{}".format(current_batch)))
...
if __name__ == "__main__":
    ...
    ## hard code cred_path # you can certain add it in the args when you trigger the script in bash
    args.cred_path = '../credentials_ft.txt'
    ######
